...
- Concurrent cancel + cancel race (e.g. an impatient double-click, or two staff acting on the same order): **this was initially under-specified here and had to be corrected after the final whole-branch review caught it.** The original claim — that `cancel()`'s status check plus the order lifecycle's `beforeUpdate` guard together prevent a double-transition — does not hold: the status check is an unguarded read outside any transaction, and `__trusted` explicitly bypasses `beforeUpdate` rather than being protected by it. Unlike `confirm()`'s decrement (which has a natural `$gte` floor and fails safely on oversell), an increment has no natural bound, so two concurrent `cancel()` calls could both pass the check and both restore stock, silently manufacturing inventory. The actual fix: the order's status flip in `cancel()` is itself a conditional `queryBuilder` compare-and-swap (`where { id, status: <expected> }`), executed first inside the transaction, so only one of two concurrent callers can ever win the row and proceed to restore stock — the loser's update affects zero rows and the call is rejected before touching any batch.
